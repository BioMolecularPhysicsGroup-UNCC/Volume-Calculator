...
HashGridVolume::HashGridVolume(bool flagOpt, float grid, float probe, float shell, float ** atomCoords, int nAtoms) 
                             : flagOpt_(flagOpt),
                               grid(grid),
                               probe(probe),
                               shell(shell),
...
void HashGridVolume::volTypeSection(int ix, int jy, int kz, int c1, int c2, int * di, int * dj, int * dk) {
    ...
    float sqDist2;                                                              //squared distance between the (x,y,z) grid point and an atom
    float closestDist2 = boundarySquared_;                                      //closest squared distance must be within boundarySquared_ 
    register float vdw;                                                         //van der Waals distance of the atom being checked. Does register help?
    ...
    register int atom;                                                          //index for an atom  ???? Does register really help? It seems it does!
    ...
    for (int c=0; c < c1 ; c++) {                                               //search 1st level of neighbors to check if spring model is necessary    
        ...
        while (atom != 0){            
            ...
            } else {                                                            //must count all atoms within compressive spring range 
                sqDist2 =- vdw;
                float distance = probeDiameter_ + vdw;                
                if (sqDist2 <= (distance * distance)) {                        
                    atomsWithin_[maxAtomsWithin++] = atom; 
                    if (distance < closeDistance) {
                        closeDistance = distance;
                        closeAtom = atom;      
                    }
                    distance = (probe/2.0 + vdw) * (probe/2.0 + vdw);
                    if (sqDist2 <= distance) {
                        if (distance < veryCloseDistance) {
                            veryCloseDistance = distance;
                            veryCloseAtom = closeAtom;
                        }
                    }
            ...
            if (sqDist2 < closestDist2){                                        //when true => must further test if isVoidNotBulk = true?
                float distance = vdw + shell;
                float r2NotBulk = distance * distance;                                  //within this squared-distance is in boundary                
                if (sqDist2 <= r2NotBulk) {                                     //condition to determine if gridpoint is bulk or not-bulk                    
                    closestDist2 = sqDist2;
                    if (distance < closeDistance) {
                        closeDistance = distance;
                        closeAtom = atom;
                    }
    ...
    if (closestDist2 > hashSpacing2_) {                                         //=> track closest atom beyond hashSpacing2_ distance 
                                                                                //search over 2nd level of neighbors for closest atom only
        for (int c = c1; c < c2; c++) {  
            int i2 = i + di[c];                                                 //Note i,j,k for hash grid,  ix, iy, kLpos_ for fine grid
            int j2 = j + dj[c];
            int k2 = k + dk[c];
            ...
            while (atom != 0) {                
                ...
                float y1 = y - atomCoords_[atom][2];
                float z1 = z - atomCoords_[atom][3];
                float sqDist2 = x1 * x1 + y1 * y1 + z1 * z1 - vdw;        
                if (sqDist2 < closestDist2){ 
                    float rNotBulk = (atomCoords_[atom][0] + shell);            //within this distance is in boundary
                    if (sqDist2 <= rNotBulk * rNotBulk){                        //checks if gridpoint is bulk or not-bulk
                        closestDist2 = sqDist2; 
                        if (rNotBulk < closeDistance) {
                            closeDistance = rNotBulk;
                            closeAtom = atom;              
                        }
                        float temp = (probe/2.0 + vdw) * (probe/2.0 + vdw);
                        if (sqDist2 <= temp) {
                            if (temp < veryCloseDistance) {
                                veryCloseDistance = temp;
                                veryCloseAtom = closeAtom;
                            }
                        }
                    }
    ...
    if (maxAtomsWithin > skipSpringMethod_) {                                   //must invoke spring model to test grid point
        int type = spring_->testSpring(x, y, z, atomsWithin_, maxAtomsWithin);  //type = -1 or 1 for micovoid or void-not-bulk
